...
// Combined endpoint to accept image URL or image file
app.MapPost("/match", async (HttpContext context, Client supabase, IHttpClientFactory httpClientFactory) =>
{
    ...
    {
        // Handle image URL in JSON body
        using var reader = new StreamReader(context.Request.Body);
        var body = await reader.ReadToEndAsync();
        var jsonDocument = JsonDocument.Parse(body);
        imageUrl = jsonDocument.RootElement.GetProperty("imageUrl").GetString();
    ...
    int topN = context.Request.Form.TryGetValue("topN", out var topNValue) && int.TryParse(topNValue, out var parsedTopN) ? parsedTopN
                : context.Request.HasFormContentType && int.TryParse(context.Request.Form["topN"], out parsedTopN) ? parsedTopN
                : 20;
