...
app.MapPost("/match-file", async (HttpContext context, Client supabase, IHttpClientFactory httpClientFactory) =>
{
    var formFile = context.Request.Form.Files.FirstOrDefault();
    if (formFile == null)
    {
        context.Response.StatusCode = StatusCodes.Status400BadRequest;
        await context.Response.WriteAsync("No image file provided");
        return;
    }

    if (!formFile.ContentType.Equals("image/png", StringComparison.OrdinalIgnoreCase))
    {
        context.Response.StatusCode = StatusCodes.Status400BadRequest;
        await context.Response.WriteAsync("Only PNG files are allowed");
        return;
    }
    ...
    int topN = context.Request.Form.TryGetValue("topN", out var topNValue) && int.TryParse(topNValue, out var parsedTopN) ? parsedTopN : 20;
...
app.MapPost("/match-url", async (HttpContext context, Client supabase, IHttpClientFactory httpClientFactory) =>
{
    ...
    {
        context.Response.StatusCode = StatusCodes.Status400BadRequest;
        await context.Response.WriteAsync("Content-Type must be application/json for URL uploads.");
        return;
    }
